...
Deno.serve(async (req) => {
  ...
  const systemPrompt = [
    ...
    "or roster, say so. Don't derive keeper eligibility from yearAcquired yourself — keeperLastYear is authoritative.",
    "",
    "When listing players (e.g. 'who can I keep through year N'):",
    "  - Filter roster on keeperLastYear >= N.",
    "  - Include the price (next$) and a one-phrase rationale per player",
    "    (e.g. '2026 minors draftee, §2(e) 4-yr contract' or '$15 callup, §2(e)').",
    "  - When a category yields zero results (e.g. 'None' in Majors), briefly explain why",
    "    (likely: all $40+ acquisitions capped to 2 add'l yrs by §2(d), or 2025-and-earlier",
    "    keepers down to yearsRemaining ≤ 2). Use the actual roster to confirm.",
    "  - Group by section: Majors, Callups, Minors. Skip empty groups except to note 'None' with the why.",
    "",
    "=== SITE GUIDE ===",
    SITE_GUIDE,
    "",
